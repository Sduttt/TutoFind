...
const PersonalDetails = (props: Props) => {
  const { data, error, nextStep, setData, signup, verifyEmail, resendEmailVerification } = useSignupStore();
  ...
  return (
    // <View className="flex-1 bg-white px-6 mt-6">

    <KeyboardAvoidingView className=' px-6 mt-6' behavior='height' keyboardVerticalOffset={100}>
      <ScrollView showsVerticalScrollIndicator={false}>

        {/* Image Upload */}
        <View className="items-center mb-8">
          <TouchableOpacity className="relative bg-gray-100 rounded-full h-28 w-28 items-center justify-center border-2 border-dashed border-gray-300">
            <FontAwesomeIcon icon={faCamera} size={32} color="#9CA3AF" />
        ...
        <View className="mb-4">
          ...
          <View className="flex-row items-center bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 focus:border-blue-500 focus:bg-white">
            ...
            <TouchableOpacity onPress={() => setShowPassword(!showPassword)} className="ml-2">
              <FontAwesomeIcon icon={showPassword ? faEyeSlash : faEye} size={18} color="#9CA3AF" />
            </TouchableOpacity>
        ...
        <View className="mb-8">
          ...
          <View className="flex-row items-center bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 focus:border-blue-500 focus:bg-white">
            ...
            <TouchableOpacity onPress={() => setShowConfirmPassword(!showConfirmPassword)} className="ml-2">
              <FontAwesomeIcon icon={showConfirmPassword ? faEyeSlash : faEye} size={18} color="#9CA3AF" />
            </TouchableOpacity>
            ...
            <View className="mt-8 items-center bg-white rounded-xl p-6 mx-4 shadow-lg">
              {
                error ? (
                  <>
                    <Text className="text-red-500 text-lg font-semibold mb-4">Signup Failed</Text>
                  ...
                  <>
                    ...
                    <Text className="text-center text-gray-700 mb-6 px-8">
                      We sent an OTP to your email address. Please enter it to verify your account.
                    </Text>
                    <TextInput
                    ...
                    {data.otp && data.otp.trim().length !== 8 && (
                      <Text className="text-red-500">Please enter a valid 8-digit OTP</Text>
                    )}
                    {!otpVerified && error && (
                      <>
                        <Text className="text-red-500 mb-2">OTP Verification Failed: {error}</Text>
                        <TouchableOpacity
                          onPress={OtpResend}
                          className="bg-button rounded-xl px-6 py-4 items-center shadow-lg active:bg-blue-700 mt-2 w-2/3"
                        >
                          <Text className="text-white font-bold text-lg">Resend OTP</Text>
                        </TouchableOpacity>
                      </>
                    )
                    }
                ...
                // TODO: ADD VALIDATION IN VERIFY BUTTON AND MAKE VERIFY FUNCTIONAL

                //TODO: ON SUCCESSFULL VERIFICATION TAKKE USER TO NEXT PAGE AND SHOW EMAIL VARIFICATION SUCCESSFULL
